...
fn main () {
    ...
    let output = match Command::new("mkpasswd").arg("-m").arg("SHA512").arg("-s").stdin(Stdio::piped()).stdout(Stdio::piped()).spawn() {
        Ok(process) => process,
        Err(error) => panic!("Unable to generate password, do you have mkpasswd installed? {}", error),
    };
    match output.stdin.unwrap().write_all(clear_passwd.as_bytes()) {
        Ok(_) => {},
